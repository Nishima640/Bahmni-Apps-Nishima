...
Bahmni.Common = Bahmni.Common || {}, Bahmni.Common.PatientSearch = Bahmni.Common.PatientSearch || {}, Bahmni.Common.PatientSearch.Constants = {
    ...
    var self = this;
    self.searchTypes = searchTypes || [], self.searchType = this.searchTypes[0], self.searchParameter = "", self.noResultsMessage = null, self.searchResults = [], self.activePatients = [], self.navigated = !1, self.links = self.searchType && self.searchType.links ? self.searchType.links : [],
        self.searchColumns = self.searchType && self.searchType.searchColumns ? self.searchType.searchColumns : ["identifier", "name"], angular.forEach(searchTypes, function(searchType) {
...
}]), angular.module("bahmni.common.conceptSet").controller("nepaliDateFieldObservationController", ["$rootScope","$scope", "conceptSetService", function($rootScope,$scope, conceptSetService) {
...
Bahmni.Common = Bahmni.Common || {}, Bahmni.Common.UIControls = Bahmni.Common.UIControls || {}, Bahmni.Common.UIControls.ProgramManagement = Bahmni.Common.UIControls.ProgramManagement || {}, angular.module("bahmni.common.uicontrols.programmanagment", []), angular.module("bahmni.common.uicontrols.programmanagment").controller("ManageProgramController", ["$scope", "retrospectiveEntryService", "$window", "programService", "spinner", "messagingService", "$stateParams", "$q", "confirmBox", "appService", function($scope, retrospectiveEntryService, $window, programService, spinner, messagingService, $stateParams, $q, confirmBox, appService) {
        ...
        $scope.confirmDeletion = function(patientProgram) {
            var scope = {};
            scope.message = "Are you sure, you want to delete " + patientProgram.display + "?", scope.cancel = _.partial(unVoidPatientProgram, patientProgram, _), scope["delete"] = _.partial(voidPatientProgram, patientProgram, _),
                confirmBox({
                    scope: scope,
                    actions: [{
                        name: "cancel",
...
Bahmni.Clinical.DrugOrderViewModel = function(config, proto, encounterDate) {
        ...
        this.changeDrug = function(drug) {
            ...
            self.frequencyType = type, self.frequencyType === Bahmni.Clinical.Constants.dosingTypes.variable ? (self.uniformDosingType.doseUnits && (self.variableDosingType.doseUnits = self.uniformDosingType.doseUnits),
                self.uniformDosingType = {}) : (self.variableDosingType.doseUnits && (self.uniformDosingType.doseUnits = self.variableDosingType.doseUnits), self.variableDosingType = {})
    ...
    var link = function($scope, element) {
        ...
        $scope.getAbbreviation = function(concept) {
            ...
            return concept && concept.mappings && concept.mappings.length > 0 && $scope.section.headingConceptSource && (result = _.result(_.find(concept.mappings, {
                ...
                result = $translate.instant(result)), result || concept.shortName || concept.name
