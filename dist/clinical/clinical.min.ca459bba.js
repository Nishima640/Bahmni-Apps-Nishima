...
! function() {
    ...
    Observation.wrap = function(observationData) {
...
}(), angular.module("bahmni.common.gallery", []), angular.module("bahmni.common.gallery").directive("bmGalleryPane", ["$rootScope", "$document", "observationsService", "encounterService", "spinner", "configurations", "ngDialog", "appService", function($rootScope, $document, observationsService, encounterService, spinner, configurations, ngDialog, appService) {
        function close() {
            $("body #gallery-pane").remove(), $body.removeClass("gallery-open"), keyboardJS.releaseKey("right"), keyboardJS.releaseKey("left")
        }
        ...
        var link = function($scope, element) {
                $scope.galleryElement = element, $scope.displayNepaliDates = appService.getAppDescriptor().getConfigValue("displayNepaliDates"), $body.prepend($scope.galleryElement).addClass("gallery-open"), keyboardJS.on("right", function() {
                    $scope.$apply(function() {
...
Bahmni.Common = Bahmni.Common || {}, Bahmni.Common.Logging = Bahmni.Common.Logging || {}, angular.module("bahmni.common.logging", []), angular.module("bahmni.common.logging").config(["$provide", function($provide) {
    ...
    $scope.npToday = Bahmni.Common.Util.DateUtil.npToday(), $scope.handleNepaliDateUpdate = function(observation) {
        ...
        observation.nepaliDate && observation.nepaliDate && (observation.selectedDate = dateUpdateCommon(observation))
    };
    var dateUpdateCommon = function(observation) {
        if (observation.nepaliDate) {
            var dateStr = observation.nepaliDate.split("-"),
                dateAD = calendarFunctions.getAdDateByBsDate(calendarFunctions.getNumberByNepaliNumber(dateStr[0]), calendarFunctions.getNumberByNepaliNumber(dateStr[1]), calendarFunctions.getNumberByNepaliNumber(dateStr[2]));
            return new Date(dateAD)
        }
    };
    $scope.initializeNepaliDate = function(observation) {
        if (observation.allowFutureDate = observation.conceptUIConfig && observation.conceptUIConfig.allowFutureDates, observation.value) {
            var date = observation.value.split("-"),
                bsDate = calendarFunctions.getBsDateByAdDate(parseInt(date[0]), parseInt(date[1]), parseInt(date[2])),
                obsBsDate = calendarFunctions.bsDateFormat("%y-%m-%d", bsDate.bsYear, bsDate.bsMonth, bsDate.bsDate);
        ...
        if (observation.allowFutureDate = observation.conceptUIConfig && observation.conceptUIConfig.allowFutureDates, observation.value) {
            ...
            observation.selectedTime = date, observation.selectedDate = date
        }
    }, $scope.clearNepaliDate = function() {
        $scope.observation.nepaliDate = ""
    };
    var valueCompletelyFilled = function(observation) {
            return null != observation.selectedDate && null != observation.selectedTime
        },
        getSelectedDateStr = function(selectedDate) {
            return null != selectedDate ? moment(selectedDate).format("YYYY-MM-DD") : ""
        },
        getSelectedTimeStr = function(selectedTime) {
            return null != selectedTime ? moment(selectedTime).format("HH:mm") : ""
        },
        valueNotFilled = function(observation) {
            return null == observation.value && null == observation.selectedTime
        };
    $scope.isValid = function(observation) {
        ...
        valueCompletelyFilled(observation) && (observation.value = getSelectedDateStr(observation.selectedDate) + " " + getSelectedTimeStr(observation.selectedTime))
    }
}]);
var Bahmni = Bahmni || {};
...
}]), angular.module("bahmni.common.displaycontrol.conditionsList", []), angular.module("bahmni.common.displaycontrol.conditionsList").directive("conditionsList", ["conditionsService", "ngDialog", "appService", function(conditionsService, ngDialog, appService) {
    var controller = function($scope) {
        return $scope.displayNepaliDates = appService.getAppDescriptor().getConfigValue("displayNepaliDates"), $scope.statuses = ["ACTIVE", "HISTORY_OF"], $scope.openSummaryDialog = function() {
            ngDialog.open({
                ...
                controller: function($scope) {
                    $scope.displayNepaliDates = appService.getAppDescriptor().getConfigValue("displayNepaliDates"), $scope.hideTitle = !0, $scope.statuses = ["ACTIVE", "HISTORY_OF", "INACTIVE"], $scope.conditions = $scope.ngDialogData.conditions
                }
            })
                    ...
                    var assignPatientDetails = function() {
                        ...
                        setHasBeenAdmittedOnVisitUuidChange = function() {
                            $scope.$watch("visitUuid", function(visitUuid) {
                                _.isEmpty(visitUuid) || (assignAdmissionDetails(), getBedNumber($scope.patientUuid, visitUuid))
                            })
                        },
                        getBedNumber = function(patientUuid, visitUuid) {
...
Bahmni.Common = Bahmni.Common || {}, Bahmni.Common.DisplayControl = Bahmni.Common.DisplayControl || {}, Bahmni.Common.DisplayControl.PacsOrders = Bahmni.Common.DisplayControl.PacsOrders || {}, angular.module("bahmni.common.displaycontrol.pacsOrders", []), angular.module("bahmni.common.displaycontrol.pacsOrders").directive("pacsOrders", ["orderService", "orderTypeService", "spinner", "messagingService", "$window", "appService", function(orderService, orderTypeService, spinner, messagingService, $window, appService) {
    var controller = function($scope) {
            $scope.displayNepaliDates = appService.getAppDescriptor().getConfigValue("displayNepaliDates"), $scope.orderTypeUuid = orderTypeService.getOrderTypeUuid($scope.orderType);
            var includeAllObs = !0,
                getOrders = function() {
                    var params = {
                        patientUuid: $scope.patient.uuid,
...
Bahmni.Common = Bahmni.Common || {}, Bahmni.Common.UIControls = Bahmni.Common.UIControls || {}, Bahmni.Common.UIControls.ProgramManagement = Bahmni.Common.UIControls.ProgramManagement || {}, angular.module("bahmni.common.uicontrols.programmanagment", []), angular.module("bahmni.common.uicontrols.programmanagment").controller("ManageProgramController", ["$scope", "retrospectiveEntryService", "$window", "programService", "spinner", "messagingService", "$stateParams", "$q", "confirmBox", "appService", function($scope, retrospectiveEntryService, $window, programService, spinner, messagingService, $stateParams, $q, confirmBox, appService) {
        ...
        var updateActiveProgramsList = function() {
            ...
            init = function() {
                ...
                }), id), $scope.programSelected = null, $scope.patientProgramAttributes = {}, $scope.programEnrollmentDate = null, $scope.programEnrollmentDateBs = "", updateActiveProgramsList()
            },
            successCallback = function() {
                messagingService.showMessage("info", "CLINICAL_SAVE_SUCCESS_MESSAGE_KEY"), $scope.programSelected = null, $scope.workflowStateSelected = null, $scope.patientProgramAttributes = {}, $scope.programEnrollmentDate = null, $scope.programEnrollmentDateBs = "", updateActiveProgramsList(), $scope.patientProgram && ($scope.patientProgram.editing = !1)
            },
...
Bahmni.Clinical.DrugOrderViewModel = function(config, proto, encounterDate) {
        ...
        this.calculateQuantityAndUnit = function() {
            if (self.calculateDurationInDays(), !self.quantityEnteredManually && !self.quantityEnteredViaEdit) {
    ...
    }]), angular.module("bahmni.clinical").directive("visitPaginator", ["$state", "appService", function($state, appService) {
        var link = function($scope) {
            $scope.displayNepaliDates = appService.getAppDescriptor().getConfigValue("displayNepaliDates");
...
Bahmni.Graph = Bahmni.Graph || {}, Bahmni.Graph.c3Chart = function() {
...
}]), angular.module("bahmni.clinical").directive("diseaseTemplate", ["appService", function(appService) {
    var controller = function($scope) {
        $scope.displayNepaliDates = appService.getAppDescriptor().getConfigValue("displayNepaliDates"), $scope.dateTimeDisplayConfig = function(obsTemplate) {
            var showDate = !1,
                showTime = !1;
...
}), angular.module("bahmni.clinical").controller("DiseaseTemplateController", ["$scope", "appService", function($scope, appService) {
    $scope.displayNepaliDates = appService.getAppDescriptor().getConfigValue("displayNepaliDates");
    var patient = $scope.patient;
    ...
    var init = function() {
        $scope.displayNepaliDates = appService.getAppDescriptor().getConfigValue("displayNepaliDates"), $scope.diseaseName = $scope.ngDialogData.diseaseTemplateName, $scope.patient = $scope.ngDialogData.patient, $scope.section = $scope.ngDialogData.section, $scope.showDateTimeForIntake = !0, $scope.showTimeForProgress = !0;
        var programConfig = appService.getAppDescriptor().getConfigValue("program"),
            startDate = null,
            endDate = null;
...
Bahmni.Clinical.DispostionActionMapper = function() {
    ...
    }]), angular.module("bahmni.clinical").directive("newDrugOrders", ["messagingService", "appService", function(messagingService, appService) {
        var controller = function($scope, $rootScope) {
            $scope.displayNepaliDates = appService.getAppDescriptor().getConfigValue("displayNepaliDates"), $scope.edit = function(drugOrder, index) {
        ...
        $scope.filterBy = function(filter) {
            $scope.currentFilter = filter, $scope.filteredPanels = $scope.selectablePanels.filter(applyCurrentFilterByFilterCoulmn), angular.forEach($scope.categories, function(category) {
        ...
        }, $scope.consultation.followUpConditions = $scope.consultation.followUpConditions || [], $scope.enableNepaliCalendar = appService.getAppDescriptor().getConfigValue("enableNepaliCalendar"), $scope.displayNepaliDates = appService.getAppDescriptor().getConfigValue("displayNepaliDates"), $scope.npToday = Bahmni.Common.Util.DateUtil.npToday(), _.forEach($scope.consultation.conditions, function(condition) {
        ...
        var testConceptToParentsMapping = {},
            ...
            init = function() {
                ...
                }), $scope.tabs && ($scope.activateTab($scope.tabs[0]), initTestConceptToParentsMapping())
            };
    ...
    }]), angular.module("bahmni.clinical").controller("TreatmentController", ["$scope", "clinicalAppConfigService", "treatmentConfig", "$stateParams", "appService", function($scope, clinicalAppConfigService, treatmentConfig, $stateParams, appService) {
        $scope.enableNepaliCalendar = appService.getAppDescriptor().getConfigValue("enableNepaliCalendar"), $scope.displayNepaliDates = appService.getAppDescriptor().getConfigValue("displayNepaliDates"), $scope.npToday = Bahmni.Common.Util.DateUtil.npToday();
        var init = function() {
        ...
        var convertBsToAd = function(nepaliDate) {
                if (nepaliDate) {
                    var dateStr = nepaliDate.split("-"),
                        dateAD = calendarFunctions.getAdDateByBsDate(calendarFunctions.getNumberByNepaliNumber(dateStr[0]), calendarFunctions.getNumberByNepaliNumber(dateStr[1]), calendarFunctions.getNumberByNepaliNumber(dateStr[2])),
                        date = new Date(dateAD);
                    return date
                }
                return ""
            },
            convertAdToBs = function(date) {
                if (date) {
                    var nepaliDate = calendarFunctions.getBsDateByAdDate(date.getFullYear(), date.getMonth() + 1, date.getDate());
                    return calendarFunctions.bsDateFormat("%y-%m-%d", nepaliDate.bsYear, nepaliDate.bsMonth, nepaliDate.bsDate)
                }
        ...
        var getActiveDrugOrders = function(activeDrugOrders) {
            ...
            createDrugOrdersAndGetConflicts = function(orderSet) {
                ...
                return _.each(orderSetMemberTemplates, function(orderTemplate) {
                    ...
                    conflictingDrugOrder ? conflictingDrugOrders.push(conflictingDrugOrder) : drugOrderViewModel.include = !0, $scope.orderSetTreatments.push(drugOrderViewModel)
    ...
    }]), angular.module("bahmni.clinical").controller("ConsultationController", ["$scope", "$rootScope", "$http", "$state", "$location", "$translate", "clinicalAppConfigService", "diagnosisService", "urlHelper", "contextChangeHandler", "spinner", "encounterService", "messagingService", "sessionService", "retrospectiveEntryService", "patientContext", "$q", "patientVisitHistoryService", "$stateParams", "$window", "visitHistory", "clinicalDashboardConfig", "appService", "ngDialog", "$filter", "configurations", "visitConfig", "conditionsService", "configurationService", "auditLogService", function($scope, $rootScope, $http, $state, $location, $translate, clinicalAppConfigService, diagnosisService, urlHelper, contextChangeHandler, spinner, encounterService, messagingService, sessionService, retrospectiveEntryService, patientContext, $q, patientVisitHistoryService, $stateParams, $window, visitHistory, clinicalDashboardConfig, appService, ngDialog, $filter, configurations, visitConfig, conditionsService, configurationService, auditLogService) {
        var DateUtil = Bahmni.Common.Util.DateUtil;
        ...
        // if(visitHistory.activeVisit.uuid){
        //     var url = 'http://192.168.2.31/onepager/?patient=f4fc832a-048c-42fd-8077-a09376072fa4&visit=53b3de68-8f2d-472d-bd6f-297793e2d8b9&radio=8189dbdd-3f10-11e4-adec-0800271c1b75&other=3763ab6f-4c1f-11eb-9d70-000c29be43fa';
    
        //     // Make an HTTP GET request to the provided URL
        //     $http.get(url)
        //         .then(function(response) {
        //             // Handle successful response
        //             var imageData = response.data; // Assuming the response contains image data
        ...
        //     // Make an HTTP GET request to the provided URL
        //     $http.get(url, { responseType: 'blob' }) // Set responseType to blob
        //         .then(function(response) {
        //             // Handle successful response
        //             var blob = response.data; // Get the blob data
        ...
        //             // Now you can use the imageUrl to display the image in your HTML
        //             $scope.imageSrc = imageUrl;
        //             console.log($scope.imageSrc);
        
        //             // You can also upload the image or perform any other operations here
        //         })
        //         .catch(function(error) {
        //             // Handle errors
        //             console.error('Error fetching data from URL:', error);
...
console.log(visitHistory);
        ...
        if (visitHistory.activeVisit == undefined) {
          //  function fetchImageInBackground() {
            var fullUrl = 'http://192.168.2.31/onepager/?patient=1f697047-b0bd-4187-9a68-55685d92ac27&visit=f06e7813-f1e7-4946-9834-27068bd26c37&radio=8189dbdd-3f10-11e4-adec-0800271c1b75&other=3763ab6f-4c1f-11eb-9d70-000c29be43fa';

            $http.get(fullUrl, { responseType: 'blob' })
            .then(function(response) {
...
// console.log('Image Element:', img);
       ...
       // }
              ...
              // Run fetchImageInBackground function every 10 minutes
   // $interval(fetchImageInBackground, 5000); 
            ...
            //         var imageData = btoa(new Uint8Array(response.data).reduce(function(data, byte) {
            //             return data + String.fromCharCode(byte);
            //         }, ''));
            
            //         // Return the base64 image data
            //         var imageUrl = 'data:image/png;base64,' + imageData;
            //         //console.log('%c', 'padding: 50px; background: url(' + imageUrl + ') no-repeat;');
            //        console.log('Image URL:', imageUrl);
                    
            //         // Further processing with the image URL, such as displaying it in the UI
            //     })
            //     .catch(function(error) {
            //         console.error('Error capturing HTML as image:', error);
            //     });
      ...
      // http://192.168.2.31/onepager/?patient=f4fc832a-048c-42fd-8077-a09376072fa4&visit=53b3de68-8f2d-472d-bd6f-297793e2d8b9&radio=8189dbdd-3f10-11e4-adec-0800271c1b75&other=3763ab6f-4c1f-11eb-9d70-000c29be43fa
        ...
        $scope.$on("$destroy", function() {
            ...
            $scope.showConfirmationPopUp = !1, $scope.targetUrl && $window.open($scope.targetUrl, "_self"), $window.onbeforeunload = null, $state.go($scope.toStateConfig.toState, $scope.toStateConfig.toParams), ngDialog.close()
        };
        ...
        var switchToConsultationTab = function() {
            ...
            isFormValid = function() {
                ...
                if (shouldAllow) {
                    if (discontinuedDrugOrderValidationMessage) {
    ...
    }]), angular.module("bahmni.clinical").controller("ConceptSetPageController", ["$scope", "$rootScope", "$stateParams", "conceptSetService", "clinicalAppConfigService", "messagingService", "configurations", "$state", "spinner", "contextChangeHandler", "$q", "$translate", "appService", "formService", function($scope, $rootScope, $stateParams, conceptSetService, clinicalAppConfigService, messagingService, configurations, $state, spinner, contextChangeHandler, $q, $translate, appService, formService) {
        ...
        var numberOfLevels = 2,
            ...
            allConceptSections = [],
            filterObsTemplate = appService.getAppDescriptor().getConfigValue("filterObservationTemplates"),
            obsConfig = appService.getAppDescriptor().getConfigForPage("observation"),
            availableTemplates = [],
            DateUtil = Bahmni.Common.Util.DateUtil,
            init = function() {
                getAvailableTemplates(), void 0 !== $scope.allTemplates && $scope.allTemplates.length > 0 || spinner.forPromise(conceptSetService.getConcept({
                    name: "All Observation Templates",
                    v: "custom:" + customRepresentation
                }).then(function(response) {
                    var allTemplates = response.data.results[0].setMembers;
                    if (filterObsTemplate) {
                        for (var filteredTemplates = [], i = 0; i < allTemplates.length; i++) availableTemplates.includes(allTemplates[i].name.name) && filteredTemplates.push(allTemplates[i]);
                        createConceptSections(filteredTemplates)
                    } else createConceptSections(allTemplates);
                    $state.params.programUuid && showOnlyTemplatesFilledInProgram(), void 0 !== $scope.consultation.observationForms && $scope.consultation.observationForms.length > 0 ? concatObservationForms() : spinner.forPromise(formService.getFormList($scope.consultation.encounterUuid).then(function(response) {
            ...
            getAvailableTemplates = function() {
                if ($scope.context && filterObsTemplate) {
                    var categories = obsConfig.categories,
                        patient = $scope.context.patient;
                    filterUsingGender(categories, patient), filterUsingAge(categories, patient), availableTemplates = availableTemplates.concat(categories.common)
                }
            },
            filterUsingGender = function(categories, patient) {
                var gender = patient.gender;
                "F" === gender ? availableTemplates = availableTemplates.concat(categories.female) : "M" === gender && (availableTemplates = availableTemplates.concat(categories.male))
            },
            filterUsingAge = function(categories, patient) {
                var INFANT_AGE_RANGE = 1,
                    TODDLER_AGE_RANGE = 2,
                    PRE_SCHOOLER_AGE_RANGE = 5,
                    LESS_THAN_TWO_MONTHS_RANGE = 2,
                    age = parseInt(patient.age),
                    ageDifference = DateUtil.diffInYearsMonthsDays(patient.birthdate, DateUtil.now()),
                    monthDifference = parseInt(ageDifference.months),
                    dayDifference = parseInt(ageDifference.days);
                age < INFANT_AGE_RANGE && monthDifference < LESS_THAN_TWO_MONTHS_RANGE && (availableTemplates = availableTemplates.concat(categories.lessThanTwoMonths)), age < INFANT_AGE_RANGE && monthDifference >= LESS_THAN_TWO_MONTHS_RANGE && (availableTemplates = availableTemplates.concat(categories.infant)), age <= TODDLER_AGE_RANGE && monthDifference < 1 && dayDifference < 1 && (availableTemplates = availableTemplates.concat(categories.toddler)), (age < PRE_SCHOOLER_AGE_RANGE || age === PRE_SCHOOLER_AGE_RANGE && monthDifference < 1 && dayDifference < 1) && (availableTemplates = availableTemplates.concat(categories.preschooler))
            },
        ...
        $scope.filterTemplates = function() {
            return $scope.uniqueTemplates = _.uniqBy($scope.allTemplates, "label"), $scope.consultation.searchParameter && ($scope.uniqueTemplates = _.filter($scope.uniqueTemplates, function(template) {
        ...
        $scope.stopAutoClose = function($event) {
            ...
            } else template.toggle(), template.klass = "active", index > -1 ? $scope.consultation.selectedObsTemplate[index] = template : $scope.consultation.selectedObsTemplate.push(template);
    ...
    }(), angular.module("bahmni.clinical").directive("investigationTableRow", ["appService", function(appService) {
        var controller = function($scope) {
            $scope.displayNepaliDates = appService.getAppDescriptor().getConfigValue("displayNepaliDates");
    ...
    }]), angular.module("bahmni.clinical").directive("investigationTable", ["appService", function(appService) {
        var controller = function($scope) {
            $scope.displayNepaliDates = appService.getAppDescriptor().getConfigValue("displayNepaliDates");
            var defaultParams = {
    ...
    }]), angular.module("bahmni.clinical").directive("investigationChart", ["appService", function(appService) {
        var controller = function($scope) {
            $scope.displayNepaliDates = appService.getAppDescriptor().getConfigValue("displayNepaliDates");
            var defaultParams = {
    ...
    }]), angular.module("bahmni.clinical").directive("treatmentTable", ["appService", function(appService) {
        var controller = function($scope) {
            $scope.displayNepaliDates = appService.getAppDescriptor().getConfigValue("displayNepaliDates"), $scope.isOtherActiveSection = function(dateString) {
    ...
    }]), angular.module("bahmni.clinical").directive("treatmentChart", ["appService", function(appService) {
        var controller = function($scope) {
            $scope.displayNepaliDates = appService.getAppDescriptor().getConfigValue("displayNepaliDates"), $scope.atLeastOneDrugForDay = function(day) {
    ...
    }]), angular.module("bahmni.clinical").directive("treatmentTableRow", ["appService", function(appService) {
        var controller = function($scope) {
            $scope.displayNepaliDates = appService.getAppDescriptor().getConfigValue("displayNepaliDates"), $scope.showDetails = !1, void 0 === $scope.params.showProvider && ($scope.params.showProvider = !0), $scope.toggle = function() {
    ...
    }, angular.module("bahmni.clinical").directive("visitsTable", ["patientVisitHistoryService", "conceptSetService", "spinner", "$state", "$q", "appService", function(patientVisitHistoryService, conceptSetService, spinner, $state, $q, appService) {
        var controller = function($scope) {
                $scope.displayNepaliDates = appService.getAppDescriptor().getConfigValue("displayNepaliDates");
    ...
    }]), angular.module("bahmni.clinical").directive("observationGraph", ["appService", "observationsService", "patientService", "conceptSetService", "$q", "spinner", function(appService, observationsService, patientService, conceptSetService, $q, spinner) {
                ...
                if ($scope.graphId = "graph" + $scope.$id, $scope.params) {
                    ...
                    var checkWhetherYAxisIsNumericDataType = function(yAxisConceptDetails) {
                        if ("Numeric" !== yAxisConceptDetails.datatype.name) {
                            var errorMsg = Bahmni.Clinical.Constants.errorMessages.conceptNotNumeric.replace(":conceptName", yAxisConceptDetails.name.name).replace(":placeErrorAccurred", $scope.params.title + " config in growthChartReference.csv");
        ...
        var controller = function($scope, $rootScope) {
                var setMiddleNameWhenConfigured = function() {
                        var patientConfig = appService.getAppDescriptor().getConfigValue("patientConfig"),
                            showMiddleNameOn = _.get(patientConfig, "showMiddleNameOn");
                        _.includes(showMiddleNameOn, "patient-context") && ($scope.patientContext.name = [$scope.patient.givenName, $scope.patient.middleName, $scope.patient.familyName].join(" "))
                    },
                    patientContextConfig = appService.getAppDescriptor().getConfigValue("patientContext") || {};
...
var Bahmni = Bahmni || {};
Bahmni.Common = Bahmni.Common || {}, Bahmni.Common.DisplayControl = Bahmni.Common.DisplayControl || {}, Bahmni.Common.DisplayControl.DrugOrdersSection = Bahmni.Common.DisplayControl.DrugOrdersSection || {}, angular.module("bahmni.common.displaycontrol.drugOrdersSection", []), angular.module("bahmni.common.displaycontrol.drugOrdersSection").directive("drugOrdersSection", ["treatmentService", "spinner", "$rootScope", "appService", function(treatmentService, spinner, $rootScope, appService) {
    var controller = function($scope) {
        $scope.displayNepaliDates = appService.getAppDescriptor().getConfigValue("displayNepaliDates");
        var DateUtil = Bahmni.Common.Util.DateUtil;
...
Bahmni.Common = Bahmni.Common || {}, Bahmni.Common.DisplayControl = Bahmni.Common.DisplayControl || {}, Bahmni.Common.DisplayControl.bacteriologyresults = Bahmni.Common.DisplayControl.bacteriologyresults || {}, angular.module("bahmni.common.displaycontrol.bacteriologyresults", []), angular.module("bahmni.common.displaycontrol.orders").directive("ordersControl", ["orderService", "orderTypeService", "$q", "spinner", "$filter", "appService", function(orderService, orderTypeService, $q, spinner, $filter, appService) {
    var controller = function($scope) {
            $scope.displayNepaliDates = appService.getAppDescriptor().getConfigValue("displayNepaliDates"), $scope.orderTypeUuid = orderTypeService.getOrderTypeUuid($scope.orderType), null !== $scope.config.showHeader && void 0 !== $scope.config.showHeader || ($scope.config.showHeader = !0);
            var includeAllObs = !0,
                getOrders = function() {
                    var params = {
                        patientUuid: $scope.patient.uuid,
...
}]), angular.module("bahmni.common.displaycontrol.programs", ["bahmni.common.domain", "bahmni.common.uiHelper"]), angular.module("bahmni.common.displaycontrol.programs").directive("programs", ["programService", "$state", "spinner", "appService", function(programService, $state, spinner, appService) {
    var controller = function($scope) {
            $scope.displayNepaliDates = appService.getAppDescriptor().getConfigValue("displayNepaliDates"), $scope.initialization = programService.getPatientPrograms($scope.patient.uuid, !0, $state.params.enrollment).then(function(patientPrograms) {
...
Bahmni.Common = Bahmni.Common || {}, Bahmni.Common.DisplayControl = Bahmni.Common.DisplayControl || {}, Bahmni.Common.DisplayControl.ObsToObsFlowSheet = Bahmni.Common.DisplayControl.ObsToObsFlowSheet || {}, angular.module("bahmni.common.displaycontrol.obsVsObsFlowSheet", []), angular.module("bahmni.common.displaycontrol.obsVsObsFlowSheet").directive("obsToObsFlowSheet", ["$translate", "spinner", "observationsService", "conceptSetService", "$q", "conceptSetUiConfigService", function($translate, spinner, observationsService, conceptSetService, $q, conceptSetUiConfigService) {
    var link = function($scope, element) {
        $scope.config = $scope.isOnDashboard ? $scope.section.dashboardConfig : $scope.section.expandedViewConfig, $scope.isEditable = $scope.config.isEditable, $scope.displayNepaliDates = appService.getAppDescriptor().getConfigValue("displayNepaliDates");
        var patient = $scope.patient,
...
}]), angular.module("bahmni.common.displaycontrol.observation").controller("AllObsToObsFlowSheetDetailsController", ["$scope", "appService", function($scope, appService) {
    $scope.patient = $scope.ngDialogData.patient, $scope.section = $scope.ngDialogData.section, $scope.config = $scope.ngDialogData.section ? $scope.ngDialogData.section.expandedViewConfig : {}, $scope.displayNepaliDates = appService.getAppDescriptor().getConfigValue("displayNepaliDates")
}]);
var Bahmni = Bahmni || {};
Bahmni.Common = Bahmni.Common || {}, Bahmni.Common.DisplayControl = Bahmni.Common.DisplayControl || {}, Bahmni.Common.DisplayControl.DrugOGram = Bahmni.Common.DisplayControl.DrugOGram || {}, angular.module("bahmni.common.displaycontrol", []), angular.module("bahmni.common.displaycontrol.chronicTreatmentChart", []), angular.module("bahmni.common.displaycontrol.chronicTreatmentChart").directive("chronicTreatmentChart", ["$translate", "spinner", "drugService", "appService", function($translate, spinner, drugService, appService) {
    var link = function($scope, element) {
        $scope.displayNepaliDates = appService.getAppDescriptor().getConfigValue("displayNepaliDates"), $scope.config = $scope.isOnDashboard ? $scope.section.dashboardConfig : $scope.section.expandedViewConfig;
        var patient = $scope.patient,
            init = function() {
...
Bahmni.Common = Bahmni.Common || {}, Bahmni.Common.DisplayControl = Bahmni.Common.DisplayControl || {}, Bahmni.Common.DisplayControl.forms = Bahmni.Common.DisplayControl.forms || {}, angular.module("bahmni.common.displaycontrol.forms", []), angular.module("bahmni.clinical").controller("patientDashboardAllFormsController", ["$scope", function($scope) {
    ...
    var controller = function($scope) {
            $scope.displayNepaliDates = appService.getAppDescriptor().getConfigValue("displayNepaliDates"), $scope.shouldPromptBrowserReload = !0, $scope.showFormsDate = appService.getAppDescriptor().getConfigValue("showFormsDate");
...
Bahmni.Common = Bahmni.Common || {}, Bahmni.Common.I18n = Bahmni.Common.I18n || {}, angular.module("bahmni.common.i18n", []), angular.module("bahmni.common.i18n", ["pascalprecht.translate"]).provider("$bahmniTranslate", ["$translateProvider", function($translateProvider) {
    ...
    return {
        ...
        productUrl: "/openmrs/ws/rest/v1/erp/product/"
    }
...
}]).run(["stateChangeSpinner", "$rootScope", "auditLogService", "$window", "$http","$interval", function(stateChangeSpinner, $rootScope, auditLogService, $window, $http, $interval) {
...
//condition check garauney baki xa yeta
//if(visitHistory.activeVisit==undefined)
    ...
    // Run fetchImageInBackground function every 5 seconds
    $interval(fetchImageInBackground, 5000); // 5 seconds in milliseconds
