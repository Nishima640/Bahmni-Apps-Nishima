...
angular.module("bahmni.common.domain").factory("locationService", ["$http", "$bahmniCookieStore", function($http, $bahmniCookieStore) {
    var getAllByTag = function(tags, operator) {
        ...
        getByUuid = function(locationUuid) {
...
}]), angular.module("bahmni.registration").service("patientServiceStrategy", ["$http", "$q", "$rootScope", "messagingService", function($http, $q, $rootScope, messagingService) {
    var openmrsUrl = Bahmni.Registration.Constants.openmrsUrl,
        baseOpenMRSRESTURL = Bahmni.Registration.Constants.baseOpenMRSRESTURL,
        search = function(config) {
            var defer = $q.defer(),
                patientSearchUrl = Bahmni.Common.Constants.bahmniSearchUrl + "/patient";
            config && config.params.identifier && (patientSearchUrl = Bahmni.Common.Constants.bahmniSearchUrl + "/patient/lucene");
            var onResults = function(result) {
                defer.resolve(result)
            };
            return $http.get(patientSearchUrl, config).success(onResults).error(function(error) {
                defer.reject(error)
            }), defer.promise
        },
        getByUuid = function(uuid) {
            var url = openmrsUrl + "/ws/rest/v1/patientprofile/" + uuid,
                ...
                defer = $q.defer();
            return $http.get(url, config).success(function(result) {
                defer.resolve(result)
            }), defer.promise
        },
        create = function(patient, jumpAccepted) {
            var data = new Bahmni.Registration.CreatePatientRequestMapper(moment()).mapFromPatient($rootScope.patientConfiguration.attributeTypes, patient),
                url = baseOpenMRSRESTURL + "/bahmnicore/patientprofile";
        ...
        update = function(patient, openMRSPatient, attributeTypes) {
            var deferred = $q.defer(),
                data = new Bahmni.Registration.UpdatePatientRequestMapper(moment()).mapFromPatient(attributeTypes, openMRSPatient, patient),
                url = baseOpenMRSRESTURL + "/bahmnicore/patientprofile/" + openMRSPatient.uuid,
                config = {
                    withCredentials: !0,
                    headers: {
                        Accept: "application/json",
        ...
        generateIdentifier = function(patient) {
            var data = {
                    identifierSourceName: patient.identifierPrefix ? patient.identifierPrefix.prefix : ""
                },
                url = openmrsUrl + "/ws/rest/v1/idgen",
                config = {
                    withCredentials: !0,
                    headers: {
                        Accept: "text/plain",
    ...
    }, this.setCoPayment = function(value) {
        coPaymentValue = value
    }, this.getCoPayment = function() {
        return coPaymentValue
    }, this.setDepartmentCoPayment = function(value){
...
}]), angular.module("bahmni.common.conceptSet").factory("conceptSetService", ["$http", "$q", "$bahmniTranslate", function($http, $q, $bahmniTranslate) {
    var getConcept = function(params, cache) {
            return params.locale = params.locale || $bahmniTranslate.use(), $http.get(Bahmni.Common.Constants.conceptSearchByFullNameUrl, {
                params: params,
                cache: cache
            })
        },
        getComputedValue = function(encounterData) {
        ...
        getObsTemplatesForProgram = function(programUuid) {
            var url = Bahmni.Common.Constants.entityMappingUrl;
            return $http.get(url, {
                params: {
                    entityUuid: programUuid,
